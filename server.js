...
app.post('/register', (req, res) => {
  ...
  bcrypt.hash(req.body.password, saltRounds, function (err, hash) {
    db.returning('user_id')
      ...
      .then(userID => {
        res.cookie('jwt', utilsFunctions.makeToken(userID), {httpOnly: true, maxAge: constants.maxAge, secure: true, sameSite:"Lax"});
        res.json(userID[0]);
      }
    )
  });
  ...
  db.select('user_id', 'password').from('users').where('email_id', '=', req.body.IDofuser).then(
    userInformation => {
      bcrypt.compare(req.body.password, userInformation[0].password, function (err, result) {
        if (result) {
          res.cookie('jwt', utilsFunctions.makeToken(userInformation[0].user_id), {httpOnly: true, maxAge:constants.maxAge, secure: true, sameSite:"Lax"});
...
app.get('/logout', (req, res) => {
  res.cookie('jwt', '', {maxAge: 1, secure: true, sameSite:"Lax", httpOnly: true});
  res.status(200).end();
});
